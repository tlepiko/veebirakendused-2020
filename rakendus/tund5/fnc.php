<?php
	...
	function resizePhoto($src, $height, $width, $keepProportion = true){
		$imageW = imagesx($src);
		$imageH = imagesy($src);
		$nWidth = $width;
		$nheight = $height;
		...
		if($width == $height){
			...
			if($imageW / $width > $imageH / $hheight){
				$nHeight = round($imageH / ($imageW / $width));
			} else {
				$nWidth = round($imageW / ($imageH / $height));
			}
		} else { //kui on vaja kindlasti etteantud suurust, ehk pisut ka kärpida
			if($imageW / $width < $imageH / $height){
				$cutSizeH = round($imageW / $width * $height);
				$cutY = round(($imageH - $cutSizeH) / 2);
			} else {
				$cutSizeW = round($imageH / $height * $width);
				$cutX = round(($imageW - $cutSizeW) / 2);
			}
		}
		if($imageW / $width > $imageH / $height){
			$imageSizeRatio = $imageW / $width;
		} else {
			$imageSizeRatio = $imageH / $height;
		}
		$nWidth = round($imageW / $imageSizeRatio);
		$nHeight = round($imageH / $imageSizeRatio);
		$myNewImage = imagecreatetruecolor($nWidth, $nHeight);
		imagecopyresampled($myNewImage, $src, 0, 0, 0, 0, $nWidth, $nHeight, $imageW, $imageH);

		// imagecopyresampled($myNewImage, $myTempImage, 0, 0, 0, 0, $nWidth, $nHeight, $imageW, $imageH);
		return $myNewImage;
	}
	...
	function saveImgToFile($myNewImage, $target, $imageFileType){
		...
		if($imageFileType == "jpg"){
			if(imagejpeg($myNewImage, $target .$fileName, 90)){
				$result = 1;
			} else {
				$result = 0;
			}
		...
		if($imageFileType == "png"){
			if(imagepng($myNewImage, $target .$fileName, 6)){
				$result = 1;
			} else {
				$result = 0;
			}
